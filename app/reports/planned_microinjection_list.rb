class PlannedMicroinjectionList
  ...
  def mi_plan_summary(consortium = nil)
    @mi_plan_summary ||= ActiveRecord::Base.connection.execute(self.class.mi_plan_summary(consortium))
  end

  def pretty_print_non_assigned_mi_plans
    Gene.pretty_print_non_assigned_mi_plans_in_bulk(nil, gene_pretty_prints['non assigned plans'])
  end

  def pretty_print_assigned_mi_plans
    Gene.pretty_print_assigned_mi_plans_in_bulk(nil, gene_pretty_prints['assigned plans'])
  end

  def pretty_print_aborted_mi_attempts
    Gene.pretty_print_mi_attempts_in_bulk_helper(nil, nil, nil, gene_pretty_prints['aborted mi attempts'])
  end

  def pretty_print_mi_attempts_in_progress
    Gene.pretty_print_mi_attempts_in_bulk_helper(nil, nil, nil, gene_pretty_prints['in progress mi attempts'])
  end

  def pretty_print_mi_attempts_genotype_confirmed
    Gene.pretty_print_mi_attempts_in_bulk_helper(nil, nil, nil, gene_pretty_prints['genotype confirmed mi attempts'])
  end
    ...
    def mi_plan_summary(consortium = nil)
      ...
      WITH mi_attempt_counts AS (
        SELECT mi_plans.id AS plan_id, SUM(CASE WHEN mi_attempt_statuses.name = 'Micro-injection aborted' THEN 1 ELSE 0 END) AS plan_aborted_count,
          ...
          FROM mi_plans
          JOIN consortia ON consortia.id = mi_plans.consortium_id #{consortium.nil? ? "" : "AND consortia.name = '#{consortium}'"}
