class NetworkGraph
  ...
  def setup
    ...
    @gene.mi_plans(:order => "created_at, id").each do |mi_plan|
      plan_no += 1
      @nodes[['MP', mi_plan.id]] = NetworkGraph::MiPlanNode.new(params={:symbol => "P#{plan_no}", :id => mi_plan.id, :consortium=> mi_plan.consortium.name, :centre=>try(mi_plan.production_centre.name).to_s, :url=>''})
      @relations << [@nodes[['G',@gene.id]], @nodes[['MP',mi_plan.id]]]
      mi_plan.mi_attempts(:order => "created_at, id").each do |mi_attempt|
        mi_no += 1
        @nodes[['MA', mi_attempt.id]] = NetworkGraph::MiAttemptNode.new(params = {:symbol => "MA#{mi_no}", :id => mi_attempt.id, :consortium=> mi_plan.consortium.name, :centre=>try(mi_plan.production_centre.name).to_s, :url=>"", :colony_background_strain => try(mi_attempt.colony_background_strain.name).to_s, :test_cross_strain => try(mi_attempt.test_cross_strain.name).to_s})
        ...
        mi_attempt.phenotype_attempts(:order => "created_at, id").each do |phenotype_attempt|
          if ! @nodes.include?(['PA',phenotype_attempt.id])
            phen_no += 1
            @nodes[['PA',phenotype_attempt.id]] = NetworkGraph::PhenotypeAttemptNode.new(params = {:symbol => "PA#{phen_no}", :id => phenotype_attempt.id, :cre_deleter_strain => try(phenotype_attempt.deleter_strain.name).to_s, :consortium=> phenotype_attempt.consortium.name, :centre=>try(phenotype_attempt.production_centre.name).to_s, :url=>""})
          end
      ...
      if ! phenotype_attempts.nil?
        phenotype_attempts.each do |phenotype_attempt|
          if ! @nodes.include?(['PA',phenotype_attempt.id])
            phen_no += 1
            @nodes[['PA',phenotype_attempt.id]] = NetworkGraph::PhenotypeAttemptNode.new(params = {:symbol => "PA#{phen_no}", :id => phenotype_attempt.id, :cre_deleter_strain => try(phenotype_attempt.deleter_strain.name).to_s, :consortium=> phenotype_attempt.consortium.name, :centre=> try(phenotype_attempt.production_centre.production_centre.name).to_s, :url=>""})
          end
