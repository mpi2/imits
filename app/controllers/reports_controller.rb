...
class ReportsController < ApplicationController
  ...
  def mi_attempts_by_gene
    ...
    @report1 = @report
    mi_attempts_by_gene5
    @report2 = @report
  end

  def mi_attempts_by_gene5
    unless params[:commit].blank?
      report = generate_mi_list_report( params )
    ...
    unless params[:commit].blank?
      ...
      grouped_report.each do |consortium|
        ...
        subgrouping.each do |production_centre|
          ...
          summary = subgrouping.subgrouping(production_centre).summary(
            'Consortium' => consortium,
            'Production Centre' => production_centre,
            '# Genes Injected' => -1,
            '# Genes Genotype Confirmed' => -1,
            '# Genes For EMMA' => -1
            #'Month Injected',
            #'# Clones Injected'           => lambda { |group| count_unique_instances_of( group, 'Clone Name' ) },
            #'# at Birth'                  => lambda { |group| count_unique_instances_of( group, 'Clone Name', lambda { |row| row.data['# Pups Born'].to_i > 0 ? true : false } ) },
...
#        puts "subgrouping: \n" + subgrouping.inspect
#        puts "data: \n" + subgrouping.data[0].inspect
#        puts "data: \n" + subgrouping.data.inspect
        
#        puts "data 2: \n" + subgrouping[ 'WTSI' ].inspect
        
#        production_centre_group = Grouping( subgrouping, :by => [ 'Production Centre' ], :order => [:name]  )       
        
#        production_centre_group.each do |production_centre|
        
#          subgrouping2 = production_centre_group.subgrouping(production_centre)
          
          @report << {
            'Consortium' => consortium,
#            'Production Centre' => subgrouping.grouped_by,    #production_centre,
            'Production Centre' => production_centre,
            '# Genes Injected' => 0,
            '# Genes Genotype Confirmed' => 0,
            '# Genes For EMMA' => 0
          }
...
#        'Consortium' => 'consortia' #lambda {'consortia'}   #{ |group| group['Marker Symbol'] },
#        'Consortium',   # => 'consortia' #lambda {'consortia'}   #{ |group| group['Marker Symbol'] },
#        'Production Centre',
#        'Consortium - Production Centre', # => lambda {|group| "#{group['Consortium']} - #{group['Production Centre']}"},
        'Consortium',
#        'Production Centre',
        '# Genes Injected'           => lambda { |group| count_unique_instances_of( group, 'Marker Symbol' ) },
        '# Genes Genotype Confirmed' => lambda { |group| count_unique_instances_of( group, 'Marker Symbol', lambda { |row| row.data['Status'] == 'Genotype confirmed' ? true : false } ) },
...
#        :order => [ 'Consortium - Production Centre', '# Genes Injected', '# Genes Genotype Confirmed' , '# Genes For EMMA']
#        :order => [ 'Consortium', 'Production Centre', '# Genes Injected', '# Genes Genotype Confirmed' , '# Genes For EMMA']
        :order => [ 'Consortium', '# Genes Injected', '# Genes Genotype Confirmed' , '# Genes For EMMA']
#        :order => [ 'Production Centre', '# Genes Injected', '# Genes Genotype Confirmed' , '# Genes For EMMA']
      )
