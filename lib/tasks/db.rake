namespace :db do
  ...
  task 'production:load' do
    ...
    if config['port'].blank?; config['port'] = '5432'; end
    system("cd #{Rails.root}; PGPASSWORD='#{config['password']}' psql -U #{config['username']} -h #{config['host']} -p #{config['port']} #{config['database']} < tmp/dump.production.sql") or raise("Failed to load production dump of DB")
  end
