...
class TargRep::GeneTrapsControllerTest < ActionController::TestCase
  ...
  context 'TargRep::GenesTrapsController' do

    setup do
      sign_in default_user
      Factory.create :gene_trap
    end
    ...
    should "allow us to GET /edit" do
      get :edit, :id => TargRep::GeneTrap.first.to_param
      assert_response :success
    end

    should "allow us to create allele, targeting vector, es cell and genbank files" do
      mol_struct    = Factory.attributes_for :gene_trap
      ...
      mol_struct_count  = TargRep::GeneTrap.count
      ...
      allele = {
        ...
        :intron             => mol_struct[:intron],

        # ES Cells only related to allele
        :es_cells => [
          Factory.attributes_for(:es_cell, :pipeline_id => 'TIGM'),
          Factory.attributes_for(:es_cell, :pipeline_id => 'TIGM'),
          Factory.attributes_for(:es_cell, :pipeline_id => 'TIGM')
        ],
      ...
      assert_equal mol_struct_count + 1, TargRep::GeneTrap.count, "Controller should have created 1 valid allele."
      assert_equal es_cell_count + 3, TargRep::EsCell.count, "Controller should have created 6 valid ES cells."
    ...
    should "allow us to create, update and delete a allele we made" do
      allele_attrs = Factory.attributes_for :gene_trap
      ...
      # CREATE
      assert_difference('TargRep::GeneTrap.count') do
        post :create, :allele => allele_attrs
      end
      assert_redirected_to targ_rep_gene_trap_path(assigns(:allele))

      created_allele = TargRep::GeneTrap.search(:gene_id_eq => allele_attrs[:gene_id]).result.last
      created_allele.save
    ...
    should "not create an invalid allele" do
      assert_no_difference('TargRep::GeneTrap.count') do
    ...
    should "show an allele" do
      allele_id = TargRep::GeneTrap.first.id
    ...
    should "find and return allele when searching by marker_symbol" do
      mol_struct = Factory.create :gene_trap, :gene => Factory.create(:gene_myolc)
    ...
    should "not allow us to update a allele with invalid parameters" do
      mol_struct_attrs = Factory.attributes_for :gene_trap
      ...
      created_mol_struct = TargRep::GeneTrap.search(:mgi_accession_id_eq => mol_struct_attrs[:mgi_accession_id]).result.first
    ...
    should "not allow us to delete a allele when we're not the creator" do
      ...
      assert_no_difference('TargRep::GeneTrap.count') do
        delete :destroy, :id => TargRep::GeneTrap.first.id
      end
      assert_response 302
    end

    should "return 404 if we try to request a genbank file that doesn't exist" do
      allele_without_gb = Factory.create :gene_trap
