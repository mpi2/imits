...
class ReportsController < ApplicationController
  ...
  def mi_attempts_monthly_production
    unless params[:commit].blank?
      ...
      if report.nil?
        redirect_to Common.cleaned_redirect_params( :mi_attempts_monthly_production, params ) if request.format == :csv
        return
      end
  ...
  def mi_attempts_by_gene
    unless params[:commit].blank?
      ...
      @report = Reports::MiAttemptsByGene::GeneSummary.get_list(request, params)

      t2 = Time.now
      @delta = t2 - t1

      #      if @report.nil?
      #        redirect_to @report = Common.cleaned_redirect_params( :mi_attempts_by_gene, params ) if request.format == :csv
      #        return
      #      end
  ...
  #    return nil if report.size == 0
  #
  #    report.add_column( '% Pups Born',                              :after => 'mi_attempts.total_pups_born' )                         { |row| Common.calculate_percentage( row.data['mi_attempts.total_pups_born'], row.data['mi_attempts.total_transferred'] ) }
  #    report.add_column( '% Total Chimeras',                         :after => 'mi_attempts.total_chimeras' )                          { |row| Common.calculate_percentage( row.data['mi_attempts.total_chimeras'], row.data['mi_attempts.total_pups_born'] ) }
  #    report.add_column( '% Male Chimeras',                          :after => 'mi_attempts.total_male_chimeras' )                     { |row| Common.calculate_percentage( row.data['mi_attempts.total_male_chimeras'], row.data['mi_attempts.total_chimeras'] ) }
  ...
  #    report.add_column( '% Chimeras With GLT',                      :after => 'mi_attempts.number_of_het_offspring' )                 { |row| Common.calculate_percentage( calculate_max_glt( row ), row.data['mi_attempts.total_male_chimeras'] ) }
  #
  #    report.remove_columns( report_column_order_and_names.dup.delete_if{ |key,value| !value.blank? }.keys )
  #    report.rename_columns( report_column_order_and_names.dup.delete_if{ |key,value| value.blank? } )
  #
  ...
  #  end
  #
  #  def Common.calculate_percentage( dividend, divisor )
  ...
  #  end
  #
  #  def Common.count_unique_instances_of( group, data_name, row_condition=nil )
  #    array = []
  #    group.each do |row|
