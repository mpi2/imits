...
#                    ['all','mouse allele modification'] => {'all' => ?, 'allele_based' => ?}
    ...
    def es_cell_and_crispr_sql
      <<-EOF
        ...
        FROM (SELECT DISTINCT mi_plans.gene_id FROM mi_plans) distinct_gene
          JOIN genes ON genes.id = distinct_gene.gene_id
          LEFT JOIN (SELECT * FROM intermediate_report_summary_by_gene WHERE intermediate_report_summary_by_gene.catagory = 'all' AND intermediate_report_summary_by_gene.approach = 'plan') AS plan_summary ON plan_summary.gene = genes.marker_symbol
          LEFT JOIN (SELECT * FROM intermediate_report_summary_by_gene WHERE intermediate_report_summary_by_gene.catagory = 'all' AND intermediate_report_summary_by_gene.approach = 'micro-injection') AS mi_production_summary ON mi_production_summary.gene = genes.marker_symbol
          LEFT JOIN (SELECT * FROM intermediate_report_summary_by_gene WHERE intermediate_report_summary_by_gene.catagory = 'all' AND intermediate_report_summary_by_gene.approach = 'mouse allele modification') AS allele_mod_production_summary ON allele_mod_production_summary.gene = genes.marker_symbol
          LEFT JOIN (SELECT * FROM intermediate_report_summary_by_gene WHERE intermediate_report_summary_by_gene.catagory = 'all' AND intermediate_report_summary_by_gene.approach = 'all') AS phenotyping_production_summary ON phenotyping_production_summary.gene = genes.marker_symbol
    ...
    def es_cell_sql
      <<-EOF
        ...
        FROM (SELECT DISTINCT mi_plans.gene_id FROM mi_plans) distinct_gene
          JOIN genes ON genes.id = distinct_gene.gene_id
          LEFT JOIN (SELECT * FROM intermediate_report_summary_by_gene WHERE intermediate_report_summary_by_gene.catagory = 'es cell' AND intermediate_report_summary_by_gene.approach = 'plan') AS plan_summary ON plan_summary.gene = genes.marker_symbol
          LEFT JOIN (SELECT * FROM intermediate_report_summary_by_gene WHERE intermediate_report_summary_by_gene.catagory = 'es cell' AND intermediate_report_summary_by_gene.approach = 'micro-injection') AS mi_production_summary ON mi_production_summary.gene = genes.marker_symbol
          LEFT JOIN (SELECT * FROM intermediate_report_summary_by_gene WHERE intermediate_report_summary_by_gene.catagory = 'es cell' AND intermediate_report_summary_by_gene.approach = 'mouse allele modification') AS allele_mod_production_summary ON allele_mod_production_summary.gene = genes.marker_symbol
          LEFT JOIN (SELECT * FROM intermediate_report_summary_by_gene WHERE intermediate_report_summary_by_gene.catagory = 'es cell' AND intermediate_report_summary_by_gene.approach = 'all') AS phenotyping_production_summary ON phenotyping_production_summary.gene = genes.marker_symbol
        WHERE phenotyping_production_summary.phenotyping_status IS NOT NULL OR
              allele_mod_production_summary.mouse_allele_mod_status IS NOT NULL OR
              mi_production_summary.mi_attempt_status IS NOT NULL OR
              plan_summary.mi_plan_status IS NOT NULL
    ...
    def crispr_sql
      <<-EOF
        ...
        FROM (SELECT DISTINCT mi_plans.gene_id FROM mi_plans) distinct_gene
          JOIN genes ON genes.id = distinct_gene.gene_id
          LEFT JOIN (SELECT * FROM intermediate_report_summary_by_gene WHERE intermediate_report_summary_by_gene.catagory = 'crispr' AND intermediate_report_summary_by_gene.approach = 'plan') AS plan_summary ON plan_summary.gene = genes.marker_symbol
          LEFT JOIN (SELECT * FROM intermediate_report_summary_by_gene WHERE intermediate_report_summary_by_gene.catagory = 'crispr' AND intermediate_report_summary_by_gene.approach = 'micro-injection') AS mi_production_summary ON mi_production_summary.gene = genes.marker_symbol
          LEFT JOIN (SELECT * FROM intermediate_report_summary_by_gene WHERE intermediate_report_summary_by_gene.catagory = 'crispr' AND intermediate_report_summary_by_gene.approach = 'mouse allele modification') AS allele_mod_production_summary ON allele_mod_production_summary.gene = genes.marker_symbol
          LEFT JOIN (SELECT * FROM intermediate_report_summary_by_gene WHERE intermediate_report_summary_by_gene.catagory = 'crispr' AND intermediate_report_summary_by_gene.approach = 'all') AS phenotyping_production_summary ON phenotyping_production_summary.gene = genes.marker_symbol
        WHERE phenotyping_production_summary.phenotyping_status IS NOT NULL OR
              allele_mod_production_summary.mouse_allele_mod_status IS NOT NULL OR
              mi_production_summary.mi_attempt_status IS NOT NULL OR
              plan_summary.mi_plan_status IS NOT NULL
