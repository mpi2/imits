...
module MiAttempt::WarningGenerator
  ...
  WARNING_MESSAGES = {
    ...
    :assigning_production_centre => 'The consortium %{consortium_name} is planning on micro-injecting this gene, but has not yet assigned a production centre.  Continuing will assign %{production_centre_name} as the production centre micro-injecting the gene on behalf of %{consortium_name}.',
  ...
  def generate_warnings
    ...
    if !potential_mi_plan
      if MiPlan.where(mi_plan_lookup_conditions.merge(:production_centre_id => nil)).first
        add_warning :assigning_production_centre
        return true
      else
