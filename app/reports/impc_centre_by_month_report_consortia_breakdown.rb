      ...
      <<-EOF
        ...
        gtc_gene_consortia AS (
          SELECT
            ...
            genes_with_plans.consortium_name as consortium,
        ...
        phenotype_consortia AS (
          SELECT
            phenotype_gene_consortia.consortium,
            SUM(CASE
            ...
            SUM(CASE
              WHEN phenotype_experiments_started_count > 0
              THEN 1 ELSE 0
            END) as phenotype_experiments_started_count
          FROM phenotype_gene_consortia
          GROUP BY
            consortium
          ORDER BY
            consortium ASC
         ...
         SELECT
           ...
           gene_goals.pc_goals as phenotype_complete_count_goal
            ...
            FROM gtc_consortia
              ...
              LEFT JOIN phenotype_consortia ON phenotype_consortia.consortium = gtc_consortia.consortium
              JOIN consortia ON consortia.name = gtc_consortia.consortium
      ...
      <<-EOF
        ...
        phenotype_gene_consortia AS (
          SELECT
            ...
            date_trunc('MONTH', phenotyping_experiments_started) as phenotyping_experiments_date
          FROM genes_with_plans
          JOIN phenotype_attempts ON genes_with_plans.mi_plan_id = phenotype_attempts.mi_plan_id AND phenotype_attempts.status_id != 1
        ...
        phenotype_consortia AS (
          SELECT
            ...
            phenotype_gene_consortia.consortium,
            SUM(CASE
              WHEN phenotype_gene_consortia.cre_date = series.date
              THEN 1
              ELSE 0
            END) as cre_excised_or_better_count,
            SUM(CASE
              WHEN phenotype_gene_consortia.ps_date = series.date
              THEN 1
              ELSE 0
            END) as phenotype_started_or_better_count,
            SUM(CASE
              WHEN phenotype_gene_consortia.pc_date = series.date
              THEN 1
              ELSE 0
            END) as phenotype_complete_count,
            SUM(CASE
              WHEN phenotype_gene_consortia.phenotyping_experiments_date = series.date
              THEN 1
              ELSE 0
            END) as phenotyping_experiments_count
          FROM phenotype_gene_consortia
          CROSS JOIN series
          GROUP BY
            series.date,
        ...
        SELECT
          ...
          phenotype_consortia.phenotype_started_or_better_count,
          phenotype_consortia.phenotype_complete_count,
          phenotype_consortia.phenotyping_experiments_count AS phenotype_experiments_started_count,
        ...
        LEFT JOIN phenotype_consortia ON phenotype_consortia.date = gtc_consortia.date AND phenotype_consortia.consortium = gtc_consortia.consortium
        JOIN consortia ON consortia.name = gtc_consortia.consortium

        LEFT JOIN (tracking_goals AS gtc_goals JOIN centres AS c1 ON c1.id = gtc_goals.production_centre_id AND c1.name = '#{centre}' )
          ON gtc_consortia.date = gtc_goals.date AND consortia.id = gtc_goals.consortium_id AND gtc_goals.goal_type = 'total_glt_clones'
        ...
        ORDER BY gtc_consortia.date ASC, gtc_consortia.consortium ASC
      EOF
    end
