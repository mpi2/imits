...
class ReportsController < ApplicationController
  ...
  def planned_microinjection_list
    #puts params.inspect
    consortium_name = ''
#    if ! params[:consortium_id].blank? &&
    if ! params[:consortium_id][0].blank? &&
    ...
    send_data_csv("planned_microinjection_list_#{consortium_name}.csv", @report.to_csv) if request.format == :csv
  end

  #def planned_microinjection_list
  #  consortium = params[:consortium_id] && params[:consortium_id].to_s.length > 0 ? Consortium.find_by_id(params[:consortium_id]) : nil
  #  consortium_name = consortium ? consortium.name : ''
  #  @report = ReportCache.find_by_name_and_format!("planned_microinjection_list_#{consortium_name}", 'csv').to_table
  #  send_data_csv("planned_microinjection_list_#{consortium_name}.csv", @report.to_csv) if request.format == :csv
  #end

  #def planned_microinjection_list_old
