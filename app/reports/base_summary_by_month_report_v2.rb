class BaseSummaryByMonthReportV2
  ...
  def intermediate_report_class
    return IntermediateReportSummaryByConsortia
  end
  ...
  def summary_by_month_sql(previous_month = false)
    ...
    sql = <<-EOF
      WITH
        goals AS (
          SELECT grants.consortium_id AS group_field_id, grant_goals.year, grant_goals.month, SUM(COALESCE(grant_goals.mi_goal, 0)) AS mi_goal, SUM(COALESCE(grant_goals.gc_goal, 0) AS gc_goal, COALESCE(SUM(grant_goals.excision_goal), 0) AS excision_goal, COALESCE(SUM(grant_goals.phenotype_goal), 0) AS phenotype_goal
            FROM grant_goals
              JOIN grants ON grant.id = grant_goals.grant_id
          GROUP BY grants.consortium_id, grant_goals.year, grant_goals.month
        ),
        ...
        group_summary AS ( #{intermediate_report_class.select_sql(@category, @approach, @allele_type)}
        ),
        series AS (
          ...
          CROSS JOIN (
            SELECT
              group_field_id,
              CASE WHEN gene_interest_date < '#{from_date}' THEN '#{from_date_minus_a_month}' ELSE gene_interest_date END AS commenece_date,
      ...
      SELECT
        ...
        goals.gc_goal AS gc_goal,
        goals.mi_goal AS mi_goal,
        goals.excision_goal,
        goals.phenotype_goal

      FROM counts
      LEFT JOIN consortia ON consortia.name = consortium
      LEFT JOIN goals ON date_part('year', date) = goals.year AND date_part('month', date) = goals.month AND group_field_id = goals.group_field_id
      ORDER BY date ASC;
    EOF
  end
