...
class ColonyController < ApplicationController
  def show
    ...
    @title = "Colony #{@colony.name} (#{@colony.try(:trace_call).try(:trace_file_file_name)})"
    marker_symbol = @colony.try(:mi_attempt).try(:mi_plan).try(:gene).try(:marker_symbol)
    @title = "Gene #{marker_symbol} - Colony #{@colony.name} (#{@colony.try(:trace_call).try(:trace_file_file_name)})" if marker_symbol
