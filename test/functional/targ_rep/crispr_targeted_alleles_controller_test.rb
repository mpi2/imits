...
class TargRep::CrisprTargetedAllelesControllerTest < ActionController::TestCase
  ...
  context 'TargRep::CrisprTargetedAllelesController' do

    setup do
      sign_in default_user
      Factory.create :crispr_targeted_allele
    end
    ...
    should "allow us to GET /edit" do
      get :edit, :id => TargRep::CrisprTargetedAllele.first.to_param
      assert_response :success
    end

    should "allow us to create allele, targeting vector, es cell and genbank files" do
      mol_struct    = Factory.attributes_for :crispr_targeted_allele
      ...
      mol_struct_count  = TargRep::CrisprTargetedAllele.count
      ...
      assert_equal mol_struct_count + 1, TargRep::CrisprTargetedAllele.count, "Controller should have created 1 valid allele."
    ...
    should "allow us to create, update and delete a allele we made" do
      allele_attrs = Factory.attributes_for :crispr_targeted_allele
      ...
      # CREATE
      assert_difference('TargRep::CrisprTargetedAllele.count') do
        post :create, :allele => allele_attrs
      end
      assert_redirected_to targ_rep_crispr_targeted_allele_path(assigns(:allele))

      created_allele = TargRep::CrisprTargetedAllele.search(:gene_id_eq => allele_attrs[:gene_id]).result.last
      created_allele.save
    ...
    should "not create genbank file database entries if the genbank file arguments are nil" do
      ...
      post :create, :allele => {
        ...
        :cassette_type      => mol_struct[:cassette_type],
        :cassette           => mol_struct[:cassette],
        :genbank_file       => { :targeting_vector => nil }
      }
    ...
    should "not create an invalid allele" do
      assert_no_difference('TargRep::CrisprTargetedAllele.count') do
    ...
    should "show an allele" do
      allele_id = TargRep::CrisprTargetedAllele.first.id
    ...
    should "find and return allele when searching by marker_symbol" do
      mol_struct = Factory.create :crispr_targeted_allele, :gene => Factory.create(:gene_myolc)
    ...
    should "not allow us to update a allele with invalid parameters" do
      mol_struct_attrs = Factory.attributes_for :crispr_targeted_allele
      ...
      created_mol_struct = TargRep::CrisprTargetedAllele.search(:mgi_accession_id_eq => mol_struct_attrs[:mgi_accession_id]).result.first
    ...
    should "not allow us to delete a allele when we're not the creator" do
      ...
      assert_no_difference('TargRep::CrisprTargetedAllele.count') do
        delete :destroy, :id => TargRep::CrisprTargetedAllele.first.id
      end
      assert_response 302
    end

    should "return 404 if we try to request a genbank file that doesn't exist" do
      allele_without_gb = Factory.create :crispr_targeted_allele
