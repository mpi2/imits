...
class Reports::MiProduction::SummaryMonthByMonthActivity
  ...
  def self.generate_original
    ...
    proxy3.set_html(prettify2(table))    
    
    return [grouped_report, grouped_report2, grouped_report3, table4, table3, proxy, proxy2, proxy3]
  end
    ...
    summary.keys.each do |year|
      ...
      month_hash.keys.each do |month|
        ...
        cons_hash.keys.each do |cons|
          ...
          string = string.gsub(/CONS_ROWSPAN/, row_count.to_s)
        end
