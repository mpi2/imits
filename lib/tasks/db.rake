namespace :db do
  ...
  ['production', 'staging'].each do |envname|
    ...
    task "#{envname}:dump", [:include_genbank_files] do |task, args|
      ...
      unless args[:include_genbank_files]
        skip_genbank_data = '-T targ_rep_genbank_files -T targ_rep_alleles_genbank_file_collections'
        create_empty_genbank_schema = '-t targ_rep_genbank_files -t targ_rep_alleles_genbank_file_collections'
      end
  ...
  task :public_dump do
    ...
    raise "Cannot find #{envname} database config" unless config
    if config['port'].blank?; config['port'] = '5432'; end
    system("cd #{Rails.root}; PGPASSWORD='#{config['password']}' pg_dump -U #{config['username']} -h #{config['host']} -p #{config['port']} --column-inserts -T email_templates -T contacts -T notifications -T targ_rep_genbank_files -T targ_rep_alleles_genbank_file_collections -T audits --no-privileges #{config['database']} > #{uploadir}/public_dump.sql") or raise("Failed to public dump #{envname} DB")
