...
class MmrrcNew
  ...
  def run
    ...
    mmrrc_centres.each do |mi_dcentre|
      ...
      raise 'multiple alleles: Probably should deal with this.' if col.alleles.count > 1
      colony_allele = col.alleles.first
      mi = col.mi_attempt
      ...
      if(colony_allele.allele_type == 'e')
        alteration = 'Targeted NonConditional'
      end
      if(colony_allele.allele_type == 'a')
        alteration = 'Knockout First'
      end
      ...
      allele_mgi_allele_id = colony_allele.mgi_allele_accession_id
