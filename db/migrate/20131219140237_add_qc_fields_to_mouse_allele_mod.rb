class AddQcFieldsToMouseAlleleMod < ActiveRecord::Migration
  ...
  def up
    QC_FIELDS.each do |qc_field|
      add_column :mouse_allele_mods, qc_field, :integer
    end

    QC_FIELDS.each do |qc_field|
      add_foreign_key :mouse_allele_mods, :qc_results, :column => qc_field
    end
  end

  def down
    QC_FIELDS.each do |qc_field|
      remove_column :mouse_allele_mods, qc_field
    end
  end
end
