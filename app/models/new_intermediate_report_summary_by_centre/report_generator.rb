module NewIntermediateReportSummaryByCentre::ReportGenerator
  class Generate
    ...
    def to_s
      "#<NewIntermediateReportSummaryByCentre::Generate size: #{size}>"
    end
    ...
    def insert_report
      begin
        sql =  <<-EOF
          ...
          TRUNCATE new_intermediate_report_summary_by_centre;

          INSERT INTO new_intermediate_report_summary_by_centre (#{self.class.columns.join(', ')}) VALUES
        EOF
    ...
    class << self
      ...
      def report_sql
        <<-EOF
          ...
          best_mi_attempts AS (
            ...
            FROM (
              ...
              JOIN (
                SELECT
                  ...
                  first_value(best_attempts_for_gene_consortia_centre_and_status.mi_attempt_id) OVER (PARTITION BY best_attempts_for_gene_consortia_centre_and_status.gene_id, best_attempts_for_gene_consortia_centre_and_status.production_centre_id) AS mi_attempt_id
                FROM (
                  SELECT
            ...
            ORDER BY gene_id, production_centre_id
          ),
          ...
          best_overall_mouse_allele_modification AS (
            ...
            FROM (
              ...
              SELECT DISTINCT attempts_join.gene_id, attempts_join.production_centre_id, mouse_allele_mods.*
              ...
              JOIN (
                SELECT
                  ...
                  first_value(best_allele_mod_for_plan_and_status.mouse_allele_mod_id)
                    OVER (PARTITION BY best_allele_mod_for_plan_and_status.gene_id, best_allele_mod_for_plan_and_status.production_centre_id) AS mouse_allele_mod_id
                FROM (
                  SELECT
                    es_cell_plans.gene_id,
                    es_cell_plans.production_centre_id,
          ...
          best_overall_phenotyping_production_by_allele AS (
            SELECT
              ...
              aborted_statuses.created_at::date as phenotype_attempt_aborted_date
            FROM (
              SELECT DISTINCT production_join.gene_id, production_join.production_centre_id, phenotyping_productions.*
              ...
              JOIN (
                SELECT
                  ...
                  first_value(phenotype_production_ordered_by_allele_and_status.phenotype_productions_id) OVER
                    (PARTITION BY phenotype_production_ordered_by_allele_and_status.gene_id, phenotype_production_ordered_by_allele_and_status.production_centre_id) AS phenotyping_production_id
                FROM (
                  SELECT
                    es_cell_plans.gene_id,
                    es_cell_plans.production_centre_id,
          ...
          best_mouse_allele_modification AS (
            ...
            FROM (
              ...
              JOIN (
                SELECT
                  ...
                  best_allele_mod_for_plan_and_status.order_by,
                  first_value(best_allele_mod_for_plan_and_status.mouse_allele_mod_id)
                    OVER (PARTITION BY best_allele_mod_for_plan_and_status.gene_id, best_allele_mod_for_plan_and_status.production_centre_id, best_allele_mod_for_plan_and_status.allele_category) AS mouse_allele_mod_id
                FROM (
                  SELECT
                    es_cell_plans.gene_id,
                    es_cell_plans.production_centre_id,
            ...
            ORDER BY best_mouse_allele_mods.gene_id, best_mouse_allele_mods.production_centre_id
          ),
          ...
          phenotyping_production_by_allele AS (
            SELECT
              ...
              aborted_statuses.created_at::date as phenotype_attempt_aborted_date
            FROM (
              SELECT DISTINCT production_join.gene_id ,production_join.production_centre_id, phenotyping_productions.*, production_join.allele_category
              FROM phenotyping_productions
              JOIN (
                SELECT
                  phenotype_production_ordered_by_allele_and_status.gene_id,
                  ...
                  phenotype_production_ordered_by_allele_and_status.order_by,
                  first_value(phenotype_production_ordered_by_allele_and_status.phenotype_productions_id) OVER
                    (PARTITION BY phenotype_production_ordered_by_allele_and_status.gene_id, phenotype_production_ordered_by_allele_and_status.production_centre_id, phenotype_production_ordered_by_allele_and_status.allele_category) AS phenotyping_production_id
                FROM (
                  SELECT
                    es_cell_plans.gene_id,
                    es_cell_plans.production_centre_id,
            ...
            ORDER BY best_phenotyping_production_by_allele.gene_id, best_phenotyping_production_by_allele.production_centre_id
          ),
            ...
            FROM (
              ...
              JOIN (
                SELECT
                  ...
                  first_value(best_plans_for_gene_consortia_centre_and_status.mi_plan_id) OVER (PARTITION BY best_plans_for_gene_consortia_centre_and_status.gene_id, best_plans_for_gene_consortia_centre_and_status.production_centre_id) AS mi_plan_id
                FROM (
                  SELECT
                    es_cell_plans.gene_id,
                    es_cell_plans.production_centre_id,
          ...
          (
          SELECT es_cell_plans.gene_id AS gene_id, es_cell_plans.production_centre_id AS production_centre_id, min(mi_plan_commence_date.mi_plan_date) AS commenece_date
            FROM es_cell_plans
          ...
          GROUP BY es_cell_plans.gene_id, es_cell_plans.production_centre_id
          )
          ...
          FROM (SELECT DISTINCT es_cell_plans.gene_id, es_cell_plans.production_centre_id FROM es_cell_plans) AS unique_gene_plans

          JOIN centres ON centres.id = unique_gene_plans.production_centre_id
          JOIN genes ON genes.id = unique_gene_plans.gene_id
          LEFT JOIN best_mi_attempts ON best_mi_attempts.gene_id = unique_gene_plans.gene_id AND  best_mi_attempts.production_centre_id = unique_gene_plans.production_centre_id
          LEFT JOIN best_mi_plans ON best_mi_plans.gene_id = unique_gene_plans.gene_id AND best_mi_plans.production_centre_id = unique_gene_plans.production_centre_id
          LEFT JOIN best_overall_mouse_allele_modification ON best_overall_mouse_allele_modification.gene_id = unique_gene_plans.gene_id AND best_overall_mouse_allele_modification.production_centre_id = unique_gene_plans.production_centre_id
          LEFT JOIN best_overall_phenotyping_production_by_allele ON best_overall_phenotyping_production_by_allele.gene_id = unique_gene_plans.gene_id AND best_overall_phenotyping_production_by_allele.production_centre_id = unique_gene_plans.production_centre_id
          LEFT JOIN best_phenotyping_production_for_tm1b ON best_phenotyping_production_for_tm1b.gene_id = unique_gene_plans.gene_id AND best_phenotyping_production_for_tm1b.production_centre_id = unique_gene_plans.production_centre_id
          LEFT JOIN best_phenotyping_production_for_tm1a ON best_phenotyping_production_for_tm1a.gene_id = unique_gene_plans.gene_id AND best_phenotyping_production_for_tm1a.production_centre_id = unique_gene_plans.production_centre_id
          LEFT JOIN best_mouse_allele_mod_for_tm1a ON best_mouse_allele_mod_for_tm1a.gene_id = unique_gene_plans.gene_id AND best_mouse_allele_mod_for_tm1a.production_centre_id = unique_gene_plans.production_centre_id
          LEFT JOIN best_mouse_allele_mod_for_tm1b ON best_mouse_allele_mod_for_tm1b.gene_id = unique_gene_plans.gene_id AND best_mouse_allele_mod_for_tm1b.production_centre_id = unique_gene_plans.production_centre_id
          LEFT JOIN gene_interest_commenced ON gene_interest_commenced.gene_id = unique_gene_plans.gene_id AND gene_interest_commenced.production_centre_id = unique_gene_plans.production_centre_id

          ORDER BY unique_gene_plans.gene_id, unique_gene_plans.production_centre_id
      ...
      def clone_efficiency_sql
        <<-EOF
          ...
          GROUP BY mi_plans.gene_id, mi_plans.production_centre_id
        EOF
      end
      ...
      def gene_efficiency_sql
        <<-EOF
          WITH counts AS (
            ...
            GROUP BY genes.id, mi_plans.production_centre_id
            ORDER BY genes.id, mi_plans.production_centre_id
          )
          ...
          FROM counts
          GROUP BY gene_id, production_centre_id
        EOF
      end
