...
namespace :reconcile_mi_attempt_distribution_centres do
  ...
  task 'run', [:respository_name, :check_reconciled, :results_csv_filepath] => [:environment] do |t, args|
    ...
    args.with_defaults(
        ...
        :results_csv_filepath => "#{Rails.application.config.paths['tmp'].first}/reports/reconcile_stats/mi_attempt_komp_results.csv"
    )
    # create instance of reconcile class and run it
      ReconcileMiAttemptDistributionCentres.new(args[:respository_name], args[:check_reconciled], args[:results_csv_filepath]).reconcile_all_mi_attempt_distribution_centres
  end
end