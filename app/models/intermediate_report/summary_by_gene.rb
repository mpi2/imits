module IntermediateReport::SummaryByGene
  class Generate < IntermediateReport::Base

     def to_s
       "#<IntermediateReportSummaryByGene::Generate size: #{size}>"
     end

     def self.table
       "intermediate_report_summary_by_gene"
     end
...
#                    '' => [],
     ...
     def self.filter_plans_sql(condition = nil)
       sql = <<-EOF
                  SELECT mi_plans.id, gene_id, consortium_id, production_centre_id, status_id FROM mi_plans #{!condition.blank? ? "WHERE mi_plans.mutagenesis_via_crispr_cas9 = #{condition}" : ''}
              EOF
       return sql
     end
     ...
     def self.best_production_sql
       # Expected to be placed inside a WITH statement which already contains the temporary tables filtered_plans and filtered_production.
         sql = <<-EOF
            best_attempts_for_gene_and_status AS (
            SELECT
          ...
          att AS (
            SELECT
              best_attempts_for_gene_and_status.gene_id,
              best_attempts_for_gene_and_status.order_by,
              first_value(best_attempts_for_gene_and_status.mi_attempt_id) OVER (PARTITION BY best_attempts_for_gene_and_status.gene_id) AS mi_attempt_id,
              first_value(best_attempts_for_gene_and_status.mouse_allele_mods_id) OVER (PARTITION BY best_attempts_for_gene_and_status.gene_id) AS mouse_allele_mod_id
            FROM best_attempts_for_gene_and_status
          ),

          top_production AS (
            SELECT DISTINCT att.gene_id, att.mi_attempt_id AS mi_attempt_id, att.mouse_allele_mod_id AS mouse_allele_mod_id
              FROM att
          )
        EOF
        return sql
     ...
     def self.best_phenotyping_sql(crispr_condition = nil, excision__condition = nil, allele_type = nil)
       sql = <<-EOF
                  ...
                  phenotype_production_for_gene_and_status AS (
                    SELECT
                      mi_plans.gene_id,
                  ...
                  phenotyping_productions_grouped AS (
                    SELECT
                      phenotype_production_for_gene_and_status.gene_id,
                      phenotype_production_for_gene_and_status.order_by,
                      first_value(phenotype_production_for_gene_and_status.phenotype_productions_id) OVER (PARTITION BY phenotype_production_for_gene_and_status.gene_id) AS phenotyping_production_id
                    FROM phenotype_production_for_gene_and_status
                  ),
                  ...
                  top_phenotyping_production AS (
                    SELECT DISTINCT phenotyping_productions_grouped.gene_id, phenotyping_productions_grouped.phenotyping_production_id
      ...
      def self.best_plans_report_sql(experiment_type,approach, plan_condition = nil)
        <<-EOF
          ...
          best_plans_for_gene_and_status AS (
            SELECT
              filtered_plans.gene_id,
              (CASE
          ...
          att AS (
            SELECT
              best_plans_for_gene_and_status.gene_id,
              best_plans_for_gene_and_status.order_by,
              first_value(best_plans_for_gene_and_status.mi_plan_id) OVER (PARTITION BY best_plans_for_gene_and_status.gene_id) AS mi_plan_id
            FROM best_plans_for_gene_and_status
          ),

          top_mi_plans AS (
            SELECT DISTINCT filtered_plans.*
              FROM filtered_plans
          ...
          SELECT
            ...
            NULL AS mouse_allele_mod_status,
            ...
            NULL AS modified_mouse_line_colony_name,
            NULL AS mouse_line_colony_name,
          ...
          ORDER BY catagory, approach, allele_type, gene_id
        EOF
      end
      ...
      def self.best_production_report_sql(experiment_type, approach, plan_condition = nil , production_condition = nil, allele_type = nil)
        <<-EOF
          ...
          SELECT
              ...
              NULL AS es_qc_in_progress_date,
              NULL AS es_qc_complete_date,
              NULL AS es_qc_fail_date,
              ...
              founder_stamps.created_at::date AS founder_obtained_date,
          ...
          LEFT JOIN (top_phenotyping_production JOIN phenotyping_productions ON phenotyping_productions.id = top_phenotyping_production.phenotyping_production_id
                    )ON top_phenotyping_production.gene_id = top_production.gene_id
          ...
          LEFT JOIN mouse_allele_mod_statuses ON mouse_allele_mods.status_id = mouse_allele_mod_statuses.id
          ...
          LEFT JOIN phenotyping_production_statuses ON phenotyping_productions.status_id = phenotyping_production_statuses.id
          ...
          ORDER BY catagory, approach, allele_type, genes.id
      ...
      def self.columns
        [
          ...
          'phenotyping_status',
          'phenotyping_registered_date',                # phenotyping_statuses
