module TargRep::AllelesHelper
  ...
  def add_es_cell_qc_conflict_link(form_builder)
    link_to_function( 'Add A QC Conflict Report', { :class => 'ss_sprite ss_add' } ) do |page|
      form_builder.fields_for :es_cell_qc_conflicts, TargRep::EsCellQcConflict.new, :child_index => 'NEW_RECORD' do |f|
        html = render( :partial => 'es_cell_qc_conflict_form', :locals => { :f => f } )
