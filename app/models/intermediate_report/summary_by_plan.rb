  ...
  class Generate < IntermediateReport::Base
    ...
    class << self
      ...
      def experiment_report_logic(data)
        data += ActiveRecord::Base.connection.execute(best_production_report_sql())
      end
      ...
      def best_production_sql
         ...
         sql = <<-EOF
          ...
          top_production AS (
            SELECT DISTINCT att.mi_plan_id, att.mi_attempt_id AS mi_attempt_id, att.mouse_allele_mod_id AS mouse_allele_mod_id
              FROM att
          )
        EOF
        return sql
      ...
      def best_production_report_sql(experiment_type, approach, plan_condition = nil , production_condition = nil)
        <<-EOF
          ...
          SELECT
              ...
              mam_registered_statuses.created_at::date AS phenotype_attempt_registered_date,
          ...
          FROM top_production
          JOIN mi_plans ON mi_plans.id = top_production.mi_plan_id
          ...
          ORDER BY catagory, approach, allele_types, genes.id, consortia.id
